...
{
	stopwatch->elapsed_time = DOPOLNI ;
}
...
uint8_t TIMUT_stopwatch_has_another_X_ms_passed(stopwatch_handle_t *stopwatch, uint32_t x)
{
	...
	if ( DOPOLNI )
	{
		...
		if (stopwatch->elapsed_time > DOPOLNI )
		{
...
void TIMUT_stopwatch_demo(void)
{
	...
	DOPOLNI ;					// postavimo časovni zaznamek s TIMUT_ funkcijo
	...
	// Najprej si na novo postavimo časovni zaznamek ("time mark").

		// DOPOLNI

	// Sedaj pa znotraj neskončne "while" zanke periodično preverjamo, ali se je že
	// iztekel trenutni interval dolžine 1 sekunda. Če se je, spremenimo stanje
	// LEDice LED1.
	while(1)
	{
		if ( TIMUT_stopwatch_has_another_X_ms_passed( DOPOLNI, DOPOLNI) )
		{
