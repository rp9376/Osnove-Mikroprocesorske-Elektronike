...
typedef struct
{
	...
	// 		function_return_type (*Function_pointer_name) (function_argument_list)
	//
	DOPOLNI		(* SetCompare) (DOPOLNI, DOPOLNI );		// kazalec na funkcijo za določanje vrednosti za primerjavo
	DOPOLNI 	(* GetCompare) (DOPOLNI);				// kazalec na funkcijo za branje vrednosti za primerjavo
...
void LCD_BKLT_init(void)
{
	...
	// 1. Inicializacija "handle" strukture za za upravljanje LCD osvetlitve
		...
		LCD_backlight.timer = DOPOLNI ;
		LCD_backlight.timer_channel = DOPOLNI ;		// namig: poglejte komentar pri implementaciji funkcije LL_TIM_CC_EnableChannel()
		...
		LCD_backlight.SetCompare = DOPOLNI;
		LCD_backlight.GetCompare = DOPOLNI;


		// --- Specifikacija privzete vrednosti za osvetlitev zaslona ---
		//
		LCD_backlight.default_brightness = DOPOLNI;		// podano v procentih
