...
void GFX_demo_objects_overlapping_delete_draw(void)
{
	...
	GFX_delete_gfx_object_on_background( &crosshair.gfx_object, &background );
...
void GFX_demo_move_one_object_delete_draw(graphic_object_t *object)
{
	...
	while(1)
	{
	  ...
	  GFX_delete_gfx_object_on_background(object, &background);
...
void GFX_demo_update_two_gfx_objects_on_background_delete_draw(graphic_object_t *front_object, graphic_object_t *middle_object, graphic_object_t *bckgnd)
{
	...
	// delete objects
	GFX_delete_gfx_object_on_background(front_object, bckgnd);
	GFX_delete_gfx_object_on_background(middle_object, bckgnd);

	// move objects
	GFX_update_gfx_object_location(front_object);	// move according to the velocity
	GFX_save_gfx_object_location(front_object);		// overwrite the old location with the current one

	GFX_update_gfx_object_location(middle_object);	// move according to the velocity
...
void GFX_demo_move_one_object_using_movement_area(graphic_object_t *object)
{
	...
	while(1)
	{
		...
		GFX_update_gfx_object_location(object);
	}
}
