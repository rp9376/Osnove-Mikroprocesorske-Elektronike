...
ErrorStatus LL_USART_ClockInit(USART_TypeDef *USARTx,
	...
	if (LL_USART_IsEnabled(USARTx) == 0U) {
		...
		assert_param(
				IS_LL_USART_CLOCKPOLARITY(USART_ClockInitStruct->ClockPolarity));
		assert_param(IS_LL_USART_CLOCKPHASE(USART_ClockInitStruct->ClockPhase));
		assert_param(
				IS_LL_USART_LASTBITCLKOUTPUT(USART_ClockInitStruct->LastBitClockPulse));
		...
		MODIFY_REG(USARTx->CR2,
				...
				USART_ClockInitStruct->ClockOutput
