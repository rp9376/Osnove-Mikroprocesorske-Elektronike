...
void GFX_display_image_array(uint16_t *img_array, int16_t x, int16_t y,
...
uint8_t GFX_are_gfx_objects_overlapping(graphic_object_t *object_A,
		graphic_object_t *object_B) {

	return GFX_are_locations_overlapping(&object_A->location,
			&object_B->location);
...
void GFX_update_two_gfx_objects_on_background(graphic_object_t *front_object,
		graphic_object_t *middle_object, graphic_object_t *bckgnd) {
...
void GFX_demo_sub_image(void) {
	...
	GFX_get_image_part(&crosshair.gfx_object.image, 0, 0, sub_x, sub_y,
			sub_image);
...
void GFX_demo_arduino_objects(void) {
	...
	GFX_draw_one_gfx_object_on_background(&arduino[MEGA].gfx_object,
			&background);

	GFX_init_gfx_object_center_location(&arduino[UNO].gfx_object, 130, 100);
	GFX_draw_one_gfx_object_on_background(&arduino[UNO].gfx_object,
			&background);

	GFX_init_gfx_object_center_location(&arduino[NANO].gfx_object, 200, 100);
	GFX_draw_one_gfx_object_on_background(&arduino[NANO].gfx_object,
			&background);
...
void GFX_demo_objects_overlapping_delete_draw(void) {
	...
	GFX_draw_two_gfx_objects_on_background(&crosshair.gfx_object,
			&arduino[MEGA].gfx_object, &background);
	...
	GFX_init_gfx_object_center_location(&crosshair.gfx_object,
			canvas.above_ground.x_center, canvas.above_ground.y_center);
	GFX_draw_two_gfx_objects_on_background(&crosshair.gfx_object,
			&arduino[MEGA].gfx_object, &background);
...
void GFX_demo_update_two_gfx_objects_on_background_delete_draw(
		graphic_object_t *front_object, graphic_object_t *middle_object,
		graphic_object_t *bckgnd) {
...
void GFX_demo_joystick_controlled_crosshair_arduino_bouncing(void) {
	...
	GFX_init_gfx_object_center_location(&arduino[UNO].gfx_object,
			arduino[UNO].gfx_object.center_limits.X_MIN,
			canvas.above_ground.y_center);
	GFX_set_gfx_object_velocity(&arduino[UNO].gfx_object, 1, 0);
	...
	//GFX_set_gfx_object_velocity(&arduino[UNO].gfx_object, 1, 1);
	...
	while (1) {
		...
		if (!GFX_update_moving_gfx_object_location(&arduino[UNO].gfx_object))
			GFX_set_gfx_object_velocity(&arduino[UNO].gfx_object,
					-arduino[UNO].gfx_object.velocity.x, 0);
