...
DOPOLNI 		keyboard;		// spremenljivka "handle" strukture za tipkovnico
...
void KBD_init(void)
{
	...
	// 1. Inicializirajte "handle" strukture za posamezne tipke v tipkovnici na strojnem nivoju
		...
		// buttons on port C
		keyboard.buttons[ DOPOLNI ].pin = DOPOLNI ;
		keyboard.buttons[ DOPOLNI ].port = DOPOLNI;
	...
	// 2. Inicializirajte začetne vrednosti "handle" strukture za posamezne tipke v tipkovnici na sistemskem nivoju
		...
		for(int i=0; i < DOPOLNI; i++)
		{
			...
			keyboard.buttons[i].state_new = DOPOLNI;
			keyboard.buttons[i].state_old = DOPOLNI;
		}
	...
	// 3. Inicializiramo še medpomnilnik tipkovnice.
		...
		BUF_init( &kbd_buf_handle, kbd_buffer, DOPOLNI);
