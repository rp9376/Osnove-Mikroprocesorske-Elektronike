...
typedef struct
{
	DOPOLNI			port;		// informacija o "portu", kamor je priključena LEDica
	uint32_t 		pin;		// informacija o pinu, kamor je priključena LEDica
...
DOPOLNI		LED_group;		// spremenljivka "handle strukture" za LED skupino
...
void LED_init(void)
{
	...
	// 1. Inicializirajte "handle strukture" za posamezne LEDice v LED skupini
		...
		// To storite tako, da pravilno inicializirate posamezne LED "handle strukture".
...
// Spodaj boste lahko videli, kako sedaj uporabimo naštevni tip LEDs_enum_t,
// in uporabimo vhodni argument funkcije "LEDn" tako, da specificiramo,
