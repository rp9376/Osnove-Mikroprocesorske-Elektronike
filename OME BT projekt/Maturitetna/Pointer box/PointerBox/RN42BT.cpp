...
bool RN42BT::begin()
{
  ...
  while(!BTserial.available()){Serial.println("waitnig..");delay(1000);}
  //delay(5000);
...
void RN42BT::sendDataByte(byte val0, byte val1)
{
  ...
  BTserial.write((byte)0x00); 
  BTserial.write((byte)0x02); // ships track because 0x0100 //it goes from back to front
...
void RN42BT::mouseMove(signed int BP_X,signed int BP_Y, byte BP_BUTTON)
{
  BTserial.write((byte)0xFD); //Start HID Report
