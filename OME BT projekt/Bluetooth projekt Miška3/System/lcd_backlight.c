...
typedef struct
{
	...
	// 		function_return_type (*Function_pointer_name) (function_argument_list)
	//
	void		(* SetCompare) (TIM_TypeDef*, uint32_t );		// kazalec na funkcijo za določanje vrednosti za primerjavo
	uint32_t 	(* GetCompare) (TIM_TypeDef*);
	...
	// kazalec na funkcijo za branje vrednosti za primerjavo
...
void LCD_BKLT_init(void)
{
	...
	// 2. Omogočitev delovanja časovnika, da prične s štetjem.
		...
		//or mby LL_TIM_CC_EnableChannel (TIM_TypeDef * TIMx, uint32_t Channels)
...
{
	// DOPOLNI
	LCD_backlight.default_brightness = LCD_BKLT_get_brightness();
	LCD_backlight.SetCompare( LCD_backlight.timer, 0 );
}
