...
typedef struct {
	// DOPOLNI done
	GPIO_TypeDef *port;		// informacija o "portu", kamor je priključena tipka
	uint32_t pin;		// informacija o pinu, kamor je priključena tipka

	//Button current and old states
...
typedef struct {
	...
	// DOPOLNI done
	button_handle_t buttons[NUM_OF_BTN];
...
void KBD_demo(void) {
	...
	if (KBD_get_button_state(BTN_OK) == BTN_SIG_VALUE_PRESSED
			&& KBD_get_button_state(BTN_ESC) == BTN_SIG_VALUE_PRESSED) {
		LEDs_write(0xFF);
		HAL_Delay(100);
		LEDs_write(0);
		HAL_Delay(100);
